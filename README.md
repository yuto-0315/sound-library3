...
小学校における音楽づくりの支援教材として開発されたWebアプリケーションです。iPadでの使用を想定して、直感的で使いやすいUIを実現しており、新たにクラウド機能によって複数の児童・生徒間での音声共有が可能になりました。
...
- **🌐 クラウド保存機能**: 作成した楽曲をクラウドに保存・共有

### 🌐 みんなで共有ページ（新機能）
- **音声のアップロード**: 録音した音声をクラス全体で共有
- **音声のダウンロード**: 他の人がアップロードした音声を自分のライブラリに追加
- **部屋システム**: 部屋番号でクラスごとに分離された共有空間
- **検索・フィルター**: タグや名前で共有音声を検索
- **重複ダウンロード防止**: 同じ音声の複数回ダウンロードを自動で防止

### 🏫 先生用管理ページ（新機能）
- **部屋管理**: 新しい部屋の作成・削除・管理
- **音声ファイル管理**: 生徒がアップロードした音声の一覧・再生・削除
- **楽曲管理**: 生徒が作成した楽曲の確認・DAWでの再現
- **班管理**: 班番号による楽曲の整理・管理
...
- **開発環境**: XAMPP（ローカル開発用）
- **API設計**: RESTful API
- **ファイル管理**: サーバーサイドファイルアップロード

### セキュリティ・制限
- **ファイルサイズ制限**: 音声ファイル最大50MB
- **対応音声形式**: MP3, WAV, WebM, OGG, MP4
- **CORS対応**: フロントエンドとAPIの連携
